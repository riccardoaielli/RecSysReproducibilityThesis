...
def _run_algorithm_fixed_hyperparameters(experiment_configuration,
    ...
    recommender_input_args_last_test = recommender_input_args.copy() # Copio l'oggetto e sostituisco il primo argomento alla riga successiva
    recommender_input_args_last_test.CONSTRUCTOR_POSITIONAL_ARGS[0] = experiment_configuration.URM_train_last_test # Unione di train e validation
...
def run_this_algorithm_experiment(dataset_name,
    ...
    result_folder_path = "result_experiments/{}/{}/".format(ALGORITHM_NAME, dataset_name) # Definisco le cartelle in cui mettere quello che mi serve
    ...
    if dataset_name == "movielens100k":  # if_else in cui scelgo quale dataset leggere
    ...
    assert_implicit_data([URM_train, URM_validation, URM_test]) # Verifica che i dati siano impliciti
    assert_disjoint_matrices([URM_train, URM_validation, URM_test]) # Verifica che i dati siano disgiunti
    ...
    cutoff_to_optimize = 20 # Dato originale dell'articolo
    cutoff_list = [5, 10, 20, 30, 40, 50, 100] # Liste su cui il modello viene valutato (questa resta sempre uguale)
    max_total_time = 14*24*60*60  # 14 days # Tempo massimo di training
    n_cases = 50 # Numero di iperparametri che vengono valutati
    n_processes = 3

    evaluator_validation = EvaluatorHoldout(URM_validation, cutoff_list=cutoff_list) # Usato per fare evaluation sul validation set
    evaluator_validation_earlystopping = EvaluatorHoldout(URM_validation, cutoff_list=[cutoff_to_optimize]) # Uguale a quello sopra ma con un solo cutoff in modo da fare early stopping
    evaluator_test = EvaluatorHoldout(URM_test, cutoff_list=cutoff_list) # Usato per fare evaluation sul test set
    ...
    if flag_article_default:
        ...
        for fold_index in range(1,6):
            ...
            try:
                ...
                _run_algorithm_fixed_hyperparameters(experiment_configuration, # Funzione che esegue il modello con gli iperparametri settati
                                                     GDE_RecommenderWrapper, # Classe del metodo che deve eseguire
                                                     all_hyperparameters,
                                                     max_epochs_for_earlystopping,
                                                     min_epochs_for_earlystopping,
                                                     fold_folder,
    ...
    if flag_baselines_tune:
        _baseline_tune(experiment_configuration, baseline_folder_path) # Ignora come è scritta. Continene la lista dei modelli che si vogliono usare come baseline del modello, deve essere ridotta a un Top popular, item knn, pure svd e matrix factorization
