...
class MMSSL(nn.Module):
    def __init__(self, n_users, n_items, embedding_dim, weight_size, dropout_list, image_feats, text_feats, config):
    ...
    def forward(self, ui_graph, iu_graph, image_ui_graph, image_iu_graph, text_ui_graph, text_iu_graph):
        ...
        u_g_embeddings = self.user_id_embedding.weight + \
            args.id_cat_rate*F.normalize(user_emb, p=2, dim=1)
        i_g_embeddings = self.item_id_embedding.weight + \
            args.id_cat_rate*F.normalize(item_emb, p=2, dim=1)
        ...
        u_g_embeddings = u_g_embeddings + args.model_cat_rate * \
            F.normalize(image_user_feats, p=2, dim=1) + \
            args.model_cat_rate*F.normalize(text_user_feats, p=2, dim=1)
        i_g_embeddings = i_g_embeddings + args.model_cat_rate * \
