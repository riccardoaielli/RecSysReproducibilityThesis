...
class Trainer(object):
    def __init__(self, data_config):
        ...
        self.task_name = "%s_%s_%s" % (datetime.now().strftime(
            '%Y-%m-%d %H:%M:%S'), args.dataset, args.cf_model,)
        ...
        self.image_ui_graph_tmp = self.text_ui_graph_tmp = torch.tensor(
            self.ui_graph_raw.todense()).to(config['device'])
        self.image_iu_graph_tmp = self.text_iu_graph_tmp = torch.tensor(
            self.ui_graph_raw.T.todense()).to(config['device'])
        ...
        self.model = MMSSL(self.n_users, self.n_items, self.emb_dim,
                           self.weight_size, self.mess_dropout, self.image_feats, self.text_feats, config)
        self.model = self.model.to(config['device'])
        self.D = Discriminator(self.n_items).to(config['device'])
        self.D.apply(self.weights_init)
        self.optim_D = optim.Adam(
            self.D.parameters(), lr=args.D_lr, betas=(0.5, 0.9))
    ...
    def gradient_penalty(self, D, xr, xf):
        ...
        alpha = torch.rand(args.batch_size*2, 1).to(config['device'])
    ...
    def weighted_sum(self, anchor, nei, co):
        ...
        res = (args.anchor_rate*an_w + (1-args.anchor_rate) *
               ne_w).reshape(-1, args.sample_num_ii, args.embed_size).sum(1)
    ...
    def sample_topk(self, u_sim, users, emb_type=None):
        ...
        topk_row = torch.tensor(np.array(users)).unsqueeze(1).repeat(
            1, args.ad_topk*args.ad_topk_multi_num).reshape(-1).int()  #
        topk_csr = csr_matrix((topk_data.detach().numpy(), (topk_row.detach(
        ).numpy(), topk_col.detach().numpy())), shape=(self.n_users, self.n_items))
        topk_g = dgl.heterograph({('user', 'ui', 'item'): topk_csr.nonzero()})
        _, topk_id = self.sampleTrainBatch_dgl(
            users, g=topk_g, sample_num=args.ad_topk, pos_id=None, g_neg=None, sample_num_neg=None)
        self.gene_fake[emb_type] = topk_id

        topk_id_u = torch.arange(len(users)).unsqueeze(
            1).repeat(1, args.ad_topk)
    ...
    def ssl_loss_calculation(self, ssl_image_logit, ssl_text_logit, ssl_common_logit):
        ssl_label_1_s2 = torch.ones(1, self.n_items).to(config['device'])
        ssl_label_0_s2 = torch.zeros(1, self.n_items).to(config['device'])
        ...
        ssl_label_1_c2 = torch.ones(1, self.n_items*2).to(config['device'])
        ssl_label_0_c2 = torch.zeros(1, self.n_items*2).to(config['device'])
    ...
    def batched_contrastive_loss(self, z1, z2, batch_size=1024):
        ...
        def f(x): return torch.exp(x / args.tau)
    ...
    def fake_gene_loss_calculation(self, u_emb, i_emb, emb_type=None):
        if self.gene_u != None:
            gene_real_loss = (-F.logsigmoid(
                (u_emb[self.gene_u]*i_emb[self.gene_real]).sum(-1)+1e-8)).mean()
            gene_fake_loss = (
                1-(-F.logsigmoid((u_emb[self.gene_u]*i_emb[self.gene_fake[emb_type]]).sum(-1)+1e-8))).mean()
    ...
    def reward_loss_calculation(self, users, re_u, re_i, topk_id, topk_p):
        self.gene_u = torch.tensor(np.array(users)).unsqueeze(
            1).repeat(1, args.ad_topk)
    ...
    def test(self, users_to_test, is_val):
        ...
        result = test_torch(ua_embeddings, ia_embeddings,
                            users_to_test, is_val)
        return result
    ...
    def train(self):
        ...
        for epoch in range(args.epoch):
            ...
            for idx in tqdm(range(n_batch)):
                ...
                u_ui = torch.tensor(self.ui_graph_raw[users].todense()).to(
                    config['device'])
                u_ui = F.softmax(u_ui - args.log_log_scale*torch.log(-torch.log(torch.empty(
                    (u_ui.shape[0], u_ui.shape[1]), dtype=torch.float32).uniform_(0, 1).to(config['device'])+1e-8)+1e-8)/args.real_data_tau, dim=1)  # 0.002
                ...
                if idx % args.T == 0 and idx != 0:
                    ...
                    _, image_ui_id = torch.topk(G_image_u_sim_detach, int(
                        self.n_items*args.m_topk_rate), dim=-1)
                    self.image_ui_index['x'] += np.array(torch.tensor(users).repeat(
                        1, int(self.n_items*args.m_topk_rate)).view(-1)).tolist()
            ...
            if args.verbose > 0:
                perf_str = 'Epoch %d [%.1fs + %.1fs]: train==[%.5f=%.5f + %.5f + %.5f], recall=[%.5f, %.5f, %.5f, %.5f], ' \
                           ...
                           (epoch, t2 - t1, t3 - t2, loss, mf_loss, emb_loss, reg_loss, ret['recall'][0], ret['recall'][1], ret['recall'][2],
                            ret['recall'][-1],
                            ret['precision'][0], ret['precision'][1], ret['precision'][2], ret[
                                'precision'][-1], ret['hit_ratio'][0], ret['hit_ratio'][1], ret['hit_ratio'][2], ret['hit_ratio'][-1],
            ...
            if ret['recall'][1] > best_recall:
                ...
                self.logger.logging("Test_Recall@%d: %.5f,  precision=[%.5f], ndcg=[%.5f]" % (eval(
                    args.Ks)[1], test_ret['recall'][1], test_ret['precision'][1], test_ret['ndcg'][1]))
                stopping_step = 0
