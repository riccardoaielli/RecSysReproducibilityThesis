...
from WWW2023.MMSSL_our_interface.MMSSL.utility.norm import build_sim, build_knn_normalized_graph
...
class MMSSL(nn.Module):
    def __init__(self, n_users, n_items, embedding_dim, weight_size, mess_dropout, image_feats, text_feats, config):
        ...
        self.common_trans = nn.Linear(
            self.config['embed_size'], self.config['embed_size'])
        ...
        self.image_feats = torch.tensor(
            image_feats).float().to(config['device'])
        self.text_feats = torch.tensor(text_feats).float().to(config['device'])
        ...
        self.dropout = nn.Dropout(p=self.config['drop_rate'])
        self.batch_norm = nn.BatchNorm1d(self.config['embed_size'])
        self.tau = 0.5

        initializer = nn.init.xavier_uniform_
        self.weight_dict = nn.ParameterDict({
            'w_q': nn.Parameter(initializer(torch.empty([self.config['embed_size'], self.config['embed_size']]))),
            'w_k': nn.Parameter(initializer(torch.empty([self.config['embed_size'], self.config['embed_size']]))),
            'w_v': nn.Parameter(initializer(torch.empty([self.config['embed_size'], self.config['embed_size']]))),
            'w_self_attention_item': nn.Parameter(initializer(torch.empty([self.config['embed_size'], self.config['embed_size']]))),
            'w_self_attention_user': nn.Parameter(initializer(torch.empty([self.config['embed_size'], self.config['embed_size']]))),
            'w_self_attention_cat': nn.Parameter(initializer(torch.empty([self.config['head_num']*self.config['embed_size'], self.config['embed_size']]))),
        })
    ...
    def matrix_to_tensor(self, cur_matrix, config):
    ...
    def multi_head_self_attention(self, trans_w, embedding_t_1, embedding_t):
        ...
        beh, N, d_h = q.shape[0], q.shape[1], self.config['embed_size'] / \
        ...
        Q = Q.reshape(beh, N, self.config['head_num'], int(
            d_h)).permute(2, 0, 1, 3)
        K = Q.reshape(beh, N, self.config['head_num'], int(
            d_h)).permute(2, 0, 1, 3)
        ...
        self.config['model_cat_rate']*F.normalize(Z, p=2, dim=2)
        return Z, att.detach()

    def forward(self, ui_graph, iu_graph, image_ui_graph, image_iu_graph, text_ui_graph, text_iu_graph):

        image_feats = image_item_feats = self.dropout(
            self.image_trans(self.image_feats))
        text_feats = text_item_feats = self.dropout(
            self.text_trans(self.text_feats))
        ...
        for i in range(self.config['layers']):
            ...
            image_user_id = self.mm(
                image_ui_graph, self.item_id_embedding.weight)
            image_item_id = self.mm(
                image_iu_graph, self.user_id_embedding.weight)

            text_user_feats = self.mm(ui_graph, text_feats)
            text_item_feats = self.mm(iu_graph, text_user_feats)

            text_user_id = self.mm(
                text_ui_graph, self.item_id_embedding.weight)
            text_item_id = self.mm(
                text_iu_graph, self.user_id_embedding.weight)
        ...
        u_g_embeddings = self.user_id_embedding.weight + \
            self.config['id_cat_rate']*F.normalize(user_emb, p=2, dim=1)
        i_g_embeddings = self.item_id_embedding.weight + \
            self.config['id_cat_rate']*F.normalize(item_emb, p=2, dim=1)

        user_emb_list = [u_g_embeddings]
        item_emb_list = [i_g_embeddings]
        for i in range(self.n_ui_layers):
            if i == (self.n_ui_layers-1):
                u_g_embeddings = self.softmax(
                    torch.mm(ui_graph, i_g_embeddings))
                i_g_embeddings = self.softmax(
                    torch.mm(iu_graph, u_g_embeddings))
        ...
        u_g_embeddings = u_g_embeddings + self.config['model_cat_rate'] * \
            F.normalize(image_user_feats, p=2, dim=1) + \
            self.config['model_cat_rate'] * \
            F.normalize(text_user_feats, p=2, dim=1)
        i_g_embeddings = i_g_embeddings + self.config['model_cat_rate'] * \
            F.normalize(image_item_feats, p=2, dim=1) + \
            self.config['model_cat_rate'] * \
            F.normalize(text_item_feats, p=2, dim=1)
