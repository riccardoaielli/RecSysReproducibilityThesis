...
class BM3(GeneralRecommender):
    def __init__(self, config, dataset):
        ...
        # load dataset info
        self.norm_adj = self.get_norm_adj_mat(dataset.inter_matrix(
            form='coo').astype(np.float32)).to(self.device)
    ...
    def calculate_loss(self, interactions):
        ...
        u_online, i_online = self.predictor(
            u_online_ori), self.predictor(i_online_ori)
        ...
        if self.t_feat is not None:
            ...
            loss_t = 1 - \
                cosine_similarity(
                    t_feat_online, i_target.detach(), dim=-1).mean()
            loss_tv = 1 - \
                cosine_similarity(
                    t_feat_online, t_feat_target.detach(), dim=-1).mean()
        if self.v_feat is not None:
            ...
            loss_v = 1 - \
                cosine_similarity(
                    v_feat_online, i_target.detach(), dim=-1).mean()
            loss_vt = 1 - \
                cosine_similarity(
                    v_feat_online, v_feat_target.detach(), dim=-1).mean()

        loss_ui = 1 - \
            cosine_similarity(u_online, i_target.detach(), dim=-1).mean()
        loss_iu = 1 - \
            cosine_similarity(i_online, u_target.detach(), dim=-1).mean()
