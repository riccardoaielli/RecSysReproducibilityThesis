...
class GDEDatasetReader(object):
    ...
    def __init__(self, dataset_name, pre_splitted_path, freeze_split=True):
        ...
        try:
            ...
            ds_train = pd.read_csv(os.path.join(
                dataset_dir, 'train_sparse.csv')).drop_duplicates()
            ds_test = pd.read_csv(os.path.join(
                dataset_dir, 'test_sparse.csv')).drop_duplicates()
            ...
            intersection_dataframe = pd.merge(
                ds_train, ds_test, on=['user', 'item'], how='inner')
            if not intersection_dataframe.empty:
                print("Dataset: {} contains {} interactions that appear both in the training and test data. Those interactions will be removed from the training data".format(
                    dataset_name, len(intersection_dataframe)))
            ...
            assert pd.merge(ds_train, ds_test, on=[
                            'user', 'item'], how='inner').empty
            ...
            URM_train, URM_validation = split_train_in_two_percentage_global_sample(
                URM_train.copy(), train_percentage=0.95)
