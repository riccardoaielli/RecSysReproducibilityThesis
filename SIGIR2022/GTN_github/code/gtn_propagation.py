...
class GeneralPropagation(MessagePassing):
    ...
    def get_incident_matrix(self, edge_index: Adj):
        ...
        row = torch.cat([torch.arange(edge_num), torch.arange(edge_num)]).to(self.device)
        col = torch.cat([row_index, col_index])
        value = torch.cat([torch.ones(edge_num), -1 * torch.ones(edge_num)]).to(self.device)
    ...
    def inc_norm(self, inc, edge_index, add_self_loops, normalize_para=-0.5):
        if add_self_loops:
            edge_index = fill_diag(edge_index.cpu(), 1.0).to(self.device)
        else:
            ...
            if isinstance(edge_index, Tensor):
                ...
                if cache is None:
                    ...
                    if self.cached:
                        ...
                        self.init_z = torch.zeros((incident_matrix.sizes()[0], x.size()[-1])).to(self.device)
                else:
