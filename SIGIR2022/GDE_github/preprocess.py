...
def cal_spectral_feature(Adj, size, type='user', largest=True, niter=5):
	...
	np.save(r'./datasets/'+dataset+r'/'+value_file_name,value.cpu().numpy())
	np.save(r'./datasets/'+dataset+r'/'+feature_file_name,vector.cpu().numpy())
...
np.save(r'./datasets/'+dataset+r'/'+'rate_sparse.npy',rate_matrix.cpu().numpy())
