...
df_train=pd.read_csv("datasets/" + dataset+r'/train_sparse.csv')
df_test=pd.read_csv("datasets/" + dataset+r'/test_sparse.csv')
...
# interaction matrix
rate_matrix=torch.Tensor(np.load("datasets/" + dataset+r'/rate_sparse.npy')).cuda()
...
class GDE(nn.Module):
	def __init__(self, user_size, item_size, beta=5.0, feature_type='smoothed', drop_out=0.1, latent_size=64, reg=0.01):
		...
		if feature_type=='smoothed':
			user_filter=self.weight_feature(torch.Tensor(np.load(r'./datasets/'+dataset+ r'/'+dataset+'_smooth_user_values.npy')).cuda())
			item_filter=self.weight_feature(torch.Tensor(np.load(r'./datasets/'+dataset+ r'/'+dataset+'_smooth_item_values.npy')).cuda())

			user_vector=torch.Tensor(np.load(r'./datasets/'+dataset+ r'/'+dataset+'_smooth_user_features.npy')).cuda()
			item_vector=torch.Tensor(np.load(r'./datasets/'+dataset+ r'/'+dataset+'_smooth_item_features.npy')).cuda()


		elif feature_type=='both':

			user_filter=torch.cat([self.weight_feature(torch.Tensor(np.load(r'./datasets/'+dataset+ r'/'+dataset+'_smooth_user_values.npy')).cuda())\
				,self.weight_feature(torch.Tensor(np.load(r'./datasets/'+dataset+ r'/'+dataset+'_rough_user_values.npy')).cuda())])

			item_filter=torch.cat([self.weight_feature(torch.Tensor(np.load(r'./datasets/'+dataset+ r'/'+dataset+'_smooth_item_values.npy')).cuda())\
				,self.weight_feature(torch.Tensor(np.load(r'./datasets/'+dataset+ r'/'+dataset+'_rough_item_values.npy')).cuda())])


			user_vector=torch.cat([torch.Tensor(np.load(r'./datasets/'+dataset+ r'/'+dataset+'_smooth_user_features.npy')).cuda(),\
				torch.Tensor(np.load(r'./datasets/'+dataset+ r'/'+dataset+'_rough_user_features.npy')).cuda()],1)


			item_vector=torch.cat([torch.Tensor(np.load(r'./datasets/'+dataset+ r'/'+dataset+'_smooth_item_features.npy')).cuda(),\
				torch.Tensor(np.load(r'./datasets/'+dataset+ r'/'+dataset+'_rough_item_features.npy')).cuda()],1)
