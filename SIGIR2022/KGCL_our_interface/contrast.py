...
def drop_edge_random(item2entities, p_drop, padding, device):
    res = dict()
...
class Contrast(nn.Module):
    def __init__(self, gcn_model, URM_train, tau, device, knowledge_graph_dropout_rate, user_interaction_dropout_rate, mix_ratio, uicontrast, kgc_enable):
        super(Contrast, self).__init__()
        self.URM_train_csr = sps.csr_matrix(URM_train)
        self.URM_train_coo = sps.coo_matrix(URM_train)
        self.gcn_model = gcn_model
        self.tau = tau
        self.device = device
        self.kg_p_drop = knowledge_graph_dropout_rate
        self.ui_p_drop = user_interaction_dropout_rate
        self.mix_ratio = mix_ratio
        self.uicontrast = uicontrast
        self.kgc_enable = kgc_enable
    ...
    def get_kg_views(self):
        ...
        view1 = drop_edge_random(kg, self.kg_p_drop, self.gcn_model.num_entities, self.device)
        view2 = drop_edge_random(kg, self.kg_p_drop, self.gcn_model.num_entities, self.device)
        return view1, view2
    ...
    def get_ui_views_pgrace(self):
        ...
        item_np = self.URM_train_coo.col.copy()
        keep_idx = list()
        ...
        user_np = self.URM_train_coo.row[keep_idx]
        ...
        g = torch.sparse.FloatTensor(index, data, torch.Size(coo.shape)).coalesce().to(self.device)
    ...
    def get_ui_views_weighted(self, item_stabilities, stab_weight):
        ...
        weights = (1-self.ui_p_drop)/torch.mean(stab_weight*kg_weights)*(stab_weight*kg_weights)
    ...
    def ui_drop_random(self, p_drop):
        ...
        keep_idx = randint_choice(len(self.URM_train_coo.nnz), size=int(len(self.URM_train_coo.nnz) * (1 - p_drop)), replace=False)
        # user_np = self.gcn_model.dataset.trainUser[keep_idx]
        # item_np = self.gcn_model.dataset.trainItem[keep_idx]
        user_np = self.URM_train_coo.row[keep_idx]
        item_np = self.URM_train_coo.col[keep_idx]
        ratings = np.ones_like(user_np, dtype=np.float32)
        tmp_adj = sp.csr_matrix((ratings, (user_np, item_np+self.gcn_model.num_users)), shape=(n_nodes, n_nodes))
        adj_mat = tmp_adj + tmp_adj.T
    ...
    def get_views(self, aug_side="both"):
        ...
        if aug_side=="ui" or not self.kgc_enable:
            kgv1, kgv2 = None, None
        else:
            kgv1, kgv2 = self.get_kg_views()
            

        if aug_side=="kg" or self.uicontrast=="NO" or self.uicontrast=="ITEM-BI":
            uiv1, uiv2 = None, None
        else:
