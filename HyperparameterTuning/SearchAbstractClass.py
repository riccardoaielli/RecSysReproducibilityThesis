    ...
    def _remove_intermediate_cases(self, cases_to_remove_list):
        ...
        if 0 in cases_to_remove_list:
            ...
            if os.path.isfile(self.output_folder_path + self.output_file_name_root + "_best_model.zip"):
                os.remove(self.output_folder_path +
                          self.output_file_name_root + "_best_model.zip")
                os.remove(self.output_folder_path +
                          self.output_file_name_root + "SearchBayesianSkopt.txt")
            ...
            for index in cases_to_remove_list:
                ...
                self.metadata_dict['result_on_earlystopping_df'].drop(
            ...
            if self.metadata_dict['hyperparameters_best_index'] >= min(cases_to_remove_list):
                ...
                if self.save_model in ["no"]:
                    validation_result = self.metadata_dict['result_on_validation_df'].xs(
                        self.cutoff_to_optimize, level=1, axis=0)
                    best_index = pd.to_numeric(
                        validation_result[self.metric_to_optimize]).idxmax()
                    ...
                    if os.path.isfile(self.output_folder_path + self.output_file_name_root + "_best_model.zip"):
                        os.remove(self.output_folder_path +
                                  self.output_file_name_root + "_best_model.zip")
                        os.remove(
                            self.output_folder_path + self.output_file_name_root + "SearchBayesianSkopt.txt")
                else:
                    raise NotImplementedError
        ...
        if os.path.isfile(self.output_folder_path + self.output_file_name_root + "_best_model_last.zip"):
            os.remove(self.output_folder_path +
                      self.output_file_name_root + "_best_model_last.zip")
